...
// Cabeçalhos do template que você baixa (visíveis p/ usuário)
const CSV_HEADERS = [
...
export default function Arquivos() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const predictionData = res.data?.results || res.data
  ...
  const predictionCounts = Array.isArray(prediction)
    ? prediction.reduce((acc, item) => {
    ...
    <div className="flex flex-col items-center gap-8">
      <Card className="w-full max-w-lg">
        <CardHeader>
          ...
          <CardDescription>The required columns are:</CardDescription>

          {/* chips só informativos */}
          <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Disposition</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Not Transit-Like Flag</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Stellar Eclipse Flag</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Centroid Offset Flag</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Ephemeris Contamination Flag</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Orbital Period (days)</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Transit Duration (hours)</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Transit Depth (ppm)</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Planetary Radius (Earth radii)</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Equilibrium Temperature (K)</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Insolation Flux (Earth flux)</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Transit Signal-to-Noise (SNR)</span>
            <span className="px-2 py-1 rounded bg-muted text-foreground font-mono">Disposition Score</span>
          </div>
        </CardHeader>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="relative group inline-block justify-self-end">
              {/* arquivo em public/ */}
              <a
                href="/exoplanetas_modelo.csv"
                download="exoplanetas_modelo.csv"
                ...
                <TableBody>
                  {prediction.map((item) => (
                    <TableRow key={item.index}>
                      ...
                      <TableCell>{Number(item.data.koi_period).toFixed(4)}</TableCell>
